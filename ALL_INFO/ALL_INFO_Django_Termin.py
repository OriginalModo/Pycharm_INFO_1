 ...
 **Path с query**:
 ...
 JSONEncoder в Django используется для преобразования объектов Python в JSON-формат, позволяя сериализовать стандартные
 типы данных, такие как списки и словари. Однако для работы с более сложными структурами данных, например,
 моделями Django, лучше использовать сериализаторы из Django REST Framework (DRF). Они предлагают:

 - Валидацию данных перед сохранением.
 - Удобную работу с вложенными моделями.
 - Гибкость в выборе полей для сериализации.
 - Поддержку различных форматов (JSON, XML, YAML).
 Таким образом, DRF-сериализаторы обеспечивают более мощные и удобные инструменты для работы с данными в веб-приложениях и API.
