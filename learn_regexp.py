...
# TODO Написать все книги которые читал и сколько раз +


# TODO Написать про миграции alembic +

# TODO Написать про pydantic   ...

# TODO Узнать об   таблцице символов Python   symtable +

# TODO Как правильно анализировать байт-код   dis +
...
# Написать задачу со временем datetime из плохого собеса +

# добавить pytests  и как запускать отдельные тесты  +

# ДОБАВИТЬ ЗАДАЧИ ИЗ ТЕЛЕГИ +

# добавить из рабочего компа сапиенс примеры про __hash__ в классах +


# x = iter([1, 2, 3, 4, 5, 6, 7, 8, 9])
# print(*zip(x, x, x))  # -> (1, 2, 3) (4, 5, 6) (7, 8, 9)
# x = iter([1, 2, 3, 4, 5, 6, 7, 8, 9])
# print(*zip(x, x, x, x))  # -> (1, 2, 3, 4) (5, 6, 7, 8)
# x = iter([1, 2, 3, 4, 5, 6, 7, 8, 9])
# print(*zip(x))  # -> (1,) (2,) (3,) (4,) (5,) (6,) (7,) (8,) (9,)


# def findMaxConsecutiveOnes(numbers: list) -> int:
#     """
#     Поиск подпоследовательности максимальной длины.
#     Если встречается два нуля подряд, то последовательность обрывается
#     :param numbers: список чисел, в котором нужно найти подпоследовательность
#     :return: длина максимальной подпоследовательности
#     """
#     # Граничные лучаи: [], [0,0]
#     if len(numbers) < 1 or (len(numbers) == 2 and numbers[0] == numbers[1] == 0):
#         return 0
#
#     if len(numbers) in [1, 2]:
#         return len(numbers)
#
#     length = 0
#     max_length = 0
...
#             max_length = max(max_length, length - 2)  # уменьшаем длину на 2, т.к. последние два символа - нули
#             length = 0
...
# проектирование бд +
# какие есть оптимизации в sql помимо индексов +

#  Расписать статусы ошибок например 201  404 и тд.
...
# НЕЛЬЗЯ ПУСТЫЕ Ф-СТРОКИ SyntaxError: f-string: empty expression not allowed
# print(f"{}") # -> SyntaxError: f-string: valid expression required before '}'

#  # ЗАПУСК ТЕСТОВ В ТЕРМИНАЛЕ  <-----     добавить сюда как запустить несколько тестов (там уже есть как запустить один)


# (или даже Protobuf/GRPC) добавляет задержки.      GRPC - добавить но сточка уже есть в файле
...
"""
Реализовать функцию, проверяющую число на полиндромность (слева-направо одинаковое).
...
# ВК ПОСМОТРЕТЬ ВОПРОСЫ ЕЩЕ РАЗ МОЖЕТ ПЕРЕПРОВЕРИТЬ
...
# ДОБАВИТЬ ЧТО ТАКОЕ LAZY-LOAD +

# НАПИСАТЬ СТАТУСЫ ЗАПРОСОВ НАПРИМЕР ТОЛЬКО ПОДРОБНО КАЖДЫЙ  +
...
# ЧТО ТАКОЕ ВЕКТОР И ВЕКТОРНЫЕ БАЗЫ ДАННЫХ  +
